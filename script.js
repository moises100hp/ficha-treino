...
async function handleRoleSelection(role) {
    ...
    onAuthStateChanged(auth, u => window.location.reload()); // Reload to apply role
}

async function handleLinkToPersonal() {
    const personalId = document.getElementById('personal-code-input').value;
    if (!personalId) {
    ...
    if (personalDocSnap.exists() && personalDocSnap.data().role === 'personal') {
        const user = auth.currentUser;
        const userDocRef = doc(db, "users", user.uid);
...
async function saveToFirestore() {
    const studentId = currentUser.role === 'personal' ? currentUser.selectedStudentId : currentUser.uid;
    if (!studentId) return;
    const docRef = doc(db, "plans", studentId);
    await setDoc(docRef, { plan: currentWorkoutData, personalId: currentUser.role === 'personal' ? currentUser.uid : currentUser.personalId });
}

function renderUI() {
    tabsContainer.innerHTML = '';
    mainContentContainer.innerHTML = '';
    const isEditable = currentUser.role === 'personal';
    ...
    Object.keys(currentWorkoutData).forEach(fichaId => {
        ...
        if(isEditable) {
            ...
            deleteFichaBtn.innerHTML = '&times;';
...
function renderFichaContent(fichaId, isEditable) {
    ...
    for(const sectionTitle in sections) {
        ...
        let deleteBtnHTML = isEditable ? `<button onclick="deleteSection('${fichaId}', '${sectionTitle}')" class="text-red-500 hover:text-red-700 font-bold p-2 text-xl" title="Remover Seção">&times;</button>` : '';
...
Object.assign(window, {
    ...
    saveNewExercise() {
        ...
        if (!newExercise.name) return alert("O nome é obrigatório.");
        currentWorkoutData[fichaId][sectionTitle].push(newExercise);
        renderFichaContent(fichaId, true);
    ...
    openModal(url) {
        ...
        try {
            ...
            console.error("URL do vídeo inválida", e);
        }
    },
    closeModal(modalId) {
