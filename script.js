...
function handleBulkCreate() {
    ...
    lines.forEach(line => {
        if (line.startsWith('## ')) {
            ...
            if (!currentWorkoutData[currentFichaName]) {
                currentWorkoutData[currentFichaName] = {};
            }
            currentSectionName = null;
        } else if (line.startsWith('# ')) {
            if (!currentFichaName) {
                currentFichaName = "Nova Ficha";
                if (!currentWorkoutData[currentFichaName]) {
                    currentWorkoutData[currentFichaName] = {};
                }
            }
            currentSectionName = line.substring(2).trim();
            if (!currentWorkoutData[currentFichaName][currentSectionName]) {
                currentWorkoutData[currentFichaName][currentSectionName] = [];
            }
        } else if (youtubeRegex.test(line)) {
            if (currentFichaName && currentSectionName) {
                const sectionExercises = currentWorkoutData[currentFichaName][currentSectionName];
            ...
            if (currentFichaName) {
                if (!currentSectionName) {
                    currentSectionName = "Exercícios Diversos";
                    if (!currentWorkoutData[currentFichaName][currentSectionName]) {
                        currentWorkoutData[currentFichaName][currentSectionName] = [];
                    }
                }
                ...
                currentWorkoutData[currentFichaName][currentSectionName].push({ name, details, video: '' });
            }
        }
    });
